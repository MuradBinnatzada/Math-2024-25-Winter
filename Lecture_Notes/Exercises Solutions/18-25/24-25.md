...
1. **Compute the Velocity:**  
   ...
   $v(t) = \dfrac{dx}{dt} = 6t - 6$.

2. **Determine When the Particle Changes Direction:**  
   Set the velocity to zero to find when the particle changes direction:  
   $6t - 6 = 0 \quad \Longrightarrow \quad t = 1.$  
   This indicates a change of direction at $t = 1$.

3. **Compute the Distance Traveled:**  
   The total distance is found by integrating the absolute value of the velocity:  
   $$
   ...
   - **For $t \in [0,1]$:**  
     ...
     $$
     \int_{0}^{1} (6 - 6t)\, dt = \left[6t - 3t^2\right]_{0}^{1} = (6 - 3) - (0) = 3.
     $$
   ...
   - **For $t \in [1,2]$:**  
     ...
     $$
     Compute the antiderivative at the endpoints:
     - At $t = 2$:  
       $3(2)^2 - 6(2) = 12 - 12 = 0.$  
     - At $t = 1$:  
       $3(1)^2 - 6(1) = 3 - 6 = -3.$  
     So,
     $$
     \left[3t^2 - 6t\right]_{1}^{2} = 0 - (-3) = 3.
     $$

4. **Total Distance:**  
   Adding the distances from both intervals:  
   $$
   D = 3 + 3 = 6.
   $$
